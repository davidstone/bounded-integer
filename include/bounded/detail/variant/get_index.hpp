...
constexpr struct get_index_t {
	template<typename Index, typename T, typename... Ts> requires(is_valid_index(types<Index>{}, T{}, Ts{}...))
	...
	template<auto n, typename... Ts> requires(is_valid_index(constant<n>, Ts{}...))
...
constexpr struct get_type_t {
	template<typename Index, typename... Ts> requires(is_valid_index(types<Index>{}, Ts{}...))
	constexpr auto operator()(types<Index> index, Ts...) const noexcept {
		return index;
	}

	template<auto n, typename T, typename... Ts> requires(is_valid_index(constant<n>, T{}, Ts{}...))
