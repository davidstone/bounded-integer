	...
	using type = decltype(
		...
		(
			...
			std::declval<bounded::integer<
				0,
				static_cast<uintmax_t>(std::numeric_limits<typename std::remove_reference_t<Range>::size_type>::max())
			>>()
		)
	);
};
...
template<typename T, BOUNDED_REQUIRES(bounded::is_bounded_integer<T>)>
