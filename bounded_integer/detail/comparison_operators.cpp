...
constexpr auto negative_one = bounded_integer::make_bounded<-1>();
...
static_assert(bounded_integer::make_bounded<int_min>() < bounded_integer::make_bounded<int_max + 1>(), "Large negative values should be less than large positive values.");

// I have to use the preprocessor here to create a string literal
#define BOUNDED_INTEGER_SINGLE_COMPARISON(op, a, b) \
	static_assert((bounded_integer::make_bounded<a>() op bounded_integer::make_bounded<b>()), "Incorrect result for (" #a ") " #op " ( "#b ")")
...
#define BOUNDED_INTEGER_COMPARISON(op, a, b, c) \
	...
	static_assert(!(bounded_integer::make_bounded<c>() op bounded_integer::make_bounded<a>()), "Incorrect result for !((" #c ") " #op " (" #a "))")
