...
static_assert(bounded_integer::make<5>() != bounded_integer::make<6>(), "5 should not equal 6");
...
constexpr auto negative_one = bounded_integer::make<-1>();
...
static_assert(bounded_integer::make<int_min>() < bounded_integer::make<int_max + 1>(), "Large negative values should be less than large positive values.");

// I have to use the preprocessor here to create a string literal
#define BOUNDED_INTEGER_SINGLE_COMPARISON(op, a, b) \
	static_assert((bounded_integer::make<a>() op bounded_integer::make<b>()), "Incorrect result for (" #a ") " #op " ( "#b ")")
...
#define BOUNDED_INTEGER_COMPARISON(op, a, b, c) \
	...
	static_assert(!(bounded_integer::make<c>() op bounded_integer::make<a>()), "Incorrect result for !((" #c ") " #op " (" #a "))")
