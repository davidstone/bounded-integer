...
constexpr auto minimum_small_capacity = (bounded::size_of<std::pair<Size, dynamic_array_data<T, Size>>> - bounded::size_of<unsigned char>) / bounded::size_of<T>;
...
struct small_buffer_optimized_vector : private lexicographical_comparison::base {
	...
	struct small_t {
		...
		static constexpr auto capacity() {
			return bounded::max(minimum_small_capacity<T, small_buffer_optimized_vector::size_type>, bounded::constant<requested_small_capacity>);
		}
	...
	private:
		...
		dynamic_array_data<T, size_type> m_data;
	};
...
private:
	constexpr auto relocate_to_small() {
		...
		auto const guard = bounded::scope_guard([&]{
			deallocate_storage(dynamic_array_data<T, size_type>(containers::data(temp), temp.capacity()));
		});
	...
	constexpr auto deallocate_large() {
		if (is_large()) {
			deallocate_storage(dynamic_array_data<T, size_type>(containers::data(m_large), m_large.capacity()));
		}
	}
	...
	constexpr auto relocate_to_large(auto const requested_capacity) {
		auto temp = allocate_storage<T, size_type>(requested_capacity);
